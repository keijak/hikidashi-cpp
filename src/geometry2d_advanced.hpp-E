...
typedef double D;      // Coordinate. doubleかlong doubleを想定
...
template <typename T>
inline bool EQ(const T& n, const T& m) {
  ...
  rep(i, n) {
    ...
    rep(j, i) {
      if (isecSS(segs[i].first, segs[i].second, segs[j].first, segs[j].second))
        ps.push_back(crosspointLL(segs[i].first, segs[i].second, segs[j].first,
                                  segs[j].second));
    }
  }
  sort(ps.begin(), ps.end());
...
vector<L> mergeSegments(vector<L> segs) {
  int n = segs.size();
  rep(i, n) if (segs[i].second < segs[i].first)
...
// S = i + b/2 - 1 (S:多角形の面積, i: 多角形の内部にある格子点の数, b:
// 辺上の格子点の数)
